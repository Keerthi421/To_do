...
drop policy if exists "lists own rows" on lists;
...
drop policy if exists "subtasks through own task" on subtasks;
...
drop policy if exists "task tags through own task" on task_tags;
...
drop policy if exists "attachments through own task" on task_attachments;
...
drop policy if exists "calendar own rows" on calendar_connections;
