...
import {
  ...
  Clock3, Filter, Flag, Folder, Inbox, Menu, MoreHorizontal, Plus,
  Search, Settings, Sparkles, Tag, Target, Trash2, X, SlidersHorizontal,
  RefreshCw, Headphones, Paperclip, Pin, CheckCircle2
} from 'lucide-react';
import './styles.css';
import './history.css';
...
function normalizeTask(task, index = 0) {
  const createdAt = task.createdAt || new Date(Date.now() - index * 60000).toISOString();
  return {
    id: task.id ?? `${Date.now()}-${index}`,
    title: task.title || 'Untitled task',
    date: task.date || 'upcoming',
    time: task.time || '',
...
function App() {
  const [tasks, setTasks] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('anyday.tasks') || 'null');
      return Array.isArray(saved) ? saved.map(normalizeTask) : initialTasks;
    } catch {
      return initialTasks;
    }
  });
  ...
  const matches = useMemo(() => tasks.filter(t =>
  ...
  const addTask = e => {
    ...
    const task = {
      ...
      title: newTask.trim(),
      date: view === 'myday' ? 'today' : 'upcoming',
      time: '', list: 'Personal', tag: '', priority: 'none', pinned: false,
      done: false, notes: '', createdAt: timestamp, updatedAt: timestamp, completedAt: null,
    };
    setTasks(v => [task, ...v]);
    setNewTask('');
    setSelected(task.id);
  ...
  const toggleTask = id => setTasks(v => v.map(t => {
    if (t.id !== id) return t;
    const done = !t.done;
    return { ...t, done, completedAt: done ? nowIso() : null, updatedAt: nowIso() };
  }));
  ...
  return <div className={dark ? 'app dark' : 'app'}>
    ...
    <aside className={mobileNav ? 'sidebar mobile-open' : 'sidebar'}>
      ...
      <div className="side-scroll">
        ...
        <div className="side-heading"><span>My lists</span><button><Plus size={19}/></button></div>
    ...
    <main className="workspace">
      <header className="topbar">
        ...
        <div className="top-icons">
          <div className="global-search"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search tasks"/></div>
      ...
      <div className="main-grid">
        ...
        <aside className="right-panel">
          ...
          {showFilters && <div className="filter-menu"><button onClick={()=>setFilter('all')}>All priorities</button><button onClick={()=>setFilter('high')}>High priority</button><button onClick={()=>setFilter('medium')}>Medium priority</button></div>}
...
function GenericView({view,tasks,allTasks,toggleTask,selectTask,addTask,newTask,setNewTask,historyStatus,setHistoryStatus,historyRange,setHistoryRange}) {
  const title = view==='next7'?'Next 7 days':view==='all'?'All my tasks':view.startsWith('list:')?view.split(':')[1]:'My Calendar';
  return <>
    <div className="generic-head"><div><h1>{title}</h1><p>{view === 'all' ? `${allTasks.length} task${allTasks.length === 1 ? '' : 's'} stored in your complete history` : 'Plan, organize and get things done.'}</p></div><button onClick={()=>setHistoryStatus(v=>v==='all'?'active':'all')}><Filter size={17}/></button></div>
    {view === 'all' ? <AllTasksHistory tasks={tasks} toggleTask={toggleTask} selectTask={selectTask} status={historyStatus} setStatus={setHistoryStatus} range={historyRange} setRange={setHistoryRange}/> : <TaskList tasks={tasks} toggleTask={toggleTask} selectTask={selectTask}/>} 
...
function AllTasksHistory({tasks,toggleTask,selectTask,status,setStatus,range,setRange}) {
  const today = startOfDay(new Date());
  const filtered = tasks.filter(task => {
    if (status === 'active' && task.done) return false;
    if (status === 'completed' && !task.done) return false;
    if (range !== 'all') {
      const created = new Date(task.createdAt || task.updatedAt || 0);
  ...
  const grouped = filtered.reduce((acc, task) => {
    const key = historyDateKey(task.createdAt || task.updatedAt);
    (acc[key] ||= []).push(task);
    return acc;
  }, {});
  ...
  return <div className="all-history">
    <div className="history-summary"><span><CheckCircle2 size={16}/> All entered tasks</span><strong>{filtered.length}</strong><span className="history-done">{filtered.filter(t=>t.done).length} completed</span></div>
    <div className="history-controls">
      <div className="history-control-group"><button className={status==='all'?'selected':''} onClick={()=>setStatus('all')}>All</button><button className={status==='active'?'selected':''} onClick={()=>setStatus('active')}>Active</button><button className={status==='completed'?'selected':''} onClick={()=>setStatus('completed')}>Completed</button></div>
      <select value={range} onChange={e=>setRange(e.target.value)} aria-label="History range"><option value="all">All time</option><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="365">Last year</option></select>
    </div>
    {!groups.length ? <div className="history-empty"><CheckCircle2 size={34}/><h3>No matching tasks</h3><p>Your complete history is preserved; adjust the filters to see older entries.</p></div> : groups.map(([key, groupTasks]) => <section className="history-group" key={key}><div className="history-heading"><span>{historyDateLabel(key)}</span><em>{groupTasks.length}</em></div>{groupTasks.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)).map(t=><TaskRow key={t.id} task={t} toggleTask={toggleTask} selectTask={selectTask}/>)}</section>)}
  </div>
}
